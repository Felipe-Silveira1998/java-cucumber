...
import com.constants.TimeOutConstants;
...
public class FrameManager extends BasePage {
    ...
    public static void findFrameRecursive(By element) throws Exception {
        ...
        for (WebElement iframe : iframes) {
            try {
                ...
                currentFrameX = oldX + Integer.parseInt(xSTR.contains(".") ? xSTR.substring(0, xSTR.indexOf(".")) : xSTR);
                currentFrameY = oldY + Integer.parseInt(ySTR.contains(".") ? ySTR.substring(0, ySTR.indexOf(".")) : ySTR);
            } catch (Exception e) {
    ...
    public static void findAndMoveToElementInsideFrames(By locator) throws Exception {
        ...
        int count = 0;
        try {
            waitVisibilityAndPresenceOfElement(By.xpath("(//iframe | //frame)[1]"), TimeOutConstants.MIN_SECONDS);
